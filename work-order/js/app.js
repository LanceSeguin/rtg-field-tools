  ...
  async function load() {
    ...
    try {
      ...
      list.innerHTML = _events.map((ev, i) => {
        ...
        return `<div class="event-card" id="ec-${i}" onclick="Cal.select(${i})" ondblclick="Cal.select(${i}); Cal.useSelected();">
...
const App = {
  ...
  showHub() { window.location = '../'; },

  showExpense() { window.location = '../expense/'; },
